...
bool CAdsArgs::GetEntity(OdDbObjectId& e)
{
    bool rc = false;
    if (m_rb && m_rb->restype == RTENAME)
    {
        if (drx_getObjectId(e, m_rb->resval.rlname))
            rc = true;
    }
    return rc;
}
...
bool CAdsArgs::GetEntityList(OdDbObjectIdArray& arr)
{
    arr.clear();
    if (m_rb && m_rb->restype == RTPICKS)
    {
        ...
        long sslen = 0;
#endif
        if (RTNORM == ads_sslength(m_rb->resval.rlname, &sslen))
        {
            ...
            {
                ads_name ename;
                if (RTNORM == sds_ssname(m_rb->resval.rlname, i, ename))
                {
                    OdDbObjectId e;
                    if (drx_getObjectId(e, ename))
                        arr.append(e);
                }
            }
        }
...
bool CAdsArgs::GetSafeArray(COleSafeArray& sa, bool bAcad)
{
    ...
    if (bAcad)
    {
        LPDISPATCH lpApp = sds_getwinapp()->GetIDispatch(TRUE);
        if (0 == lpApp)
            return false;
...
{
    return sds_findfile(fname, result);
}
