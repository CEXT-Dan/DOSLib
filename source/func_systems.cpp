...
int CDOSLibApp::ads_dos_pause()
{
    ...
    if (nSeconds > 0)
    {
        ...
        if (pApp)
        {
            ...
            if (pWnd)
            {
                ULONGLONG dwMilliseconds = (ULONGLONG)(nSeconds * 1000);
                ...
                } while ((::GetTickCount64() - dwStart) < dwMilliseconds);
            }
        }
    }
...
{
    acedRetReal((ads_real)::GetTickCount64());
    return RSRSLT;
}
...
int CDOSLibApp::ads_dos_printscrn()
{
    ...
    else if (2 == type)
    {
        ...
        rc = image.CaptureWindow(sds_getviewhwnd());
    }
    else if (3 == type)
    {
