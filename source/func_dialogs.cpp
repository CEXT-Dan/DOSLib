...
int CDOSLibApp::ads_dos_getint()
{
    ...
    CDosGetIntDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
...
int CDOSLibApp::ads_dos_getreal()
{
    ...
    CDosGetRealDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
...
int CDOSLibApp::ads_dos_getstring()
{
    ...
    CDosGetStringDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
...
int CDOSLibApp::ads_dos_getpassword()
{
    ...
    CDosGetPwdDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_Title = strTitle;
    dlg.m_Prompt = strPrompt;
    dlg.m_String = strString;
    dlg.m_Length = nLength;
...
int CDOSLibApp::ads_dos_getcolor()
{
    ...
    CDosColorDialog dlg(0, 0, CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_Title = strTitle;
...
int CDOSLibApp::ads_dos_htmlbox()
{
    ...
    CDosHtmlDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
...
int CDOSLibApp::ads_dos_checklist()
{
    ...
    CDosCheckListDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
...
int CDOSLibApp::ads_dos_listbox()
{
    ...
    CDosListBoxDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_Title = strTitle;
    dlg.m_Prompt = strPrompt;
...
int CDOSLibApp::ads_dos_dwgpreview()
{
    ...
    CDosDwgPreviewDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_Title = strTitle;
    dlg.m_Color = nColor;
...
int CDOSLibApp::ads_dos_editbox()
{
    ...
    CDosEditBoxDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_Title = strTitle;
    dlg.m_Prompt = strPrompt;
    dlg.m_String = strString;
    if (dlg.DoModal() == IDOK)
...
int CDOSLibApp::ads_dos_proplist()
{
    ...
    CDosPropListDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
...
int CDOSLibApp::ads_dos_multilist()
{
    ...
    CDosMultiListDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_title = strTitle;
    dlg.m_prompt = strPrompt;
...
int CDOSLibApp::ads_dos_combolist()
{
    ...
    CDosComboListDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_Title = strTitle;
    dlg.m_Prompt = strPrompt;
...
int CDOSLibApp::ads_dos_editlist()
{
    ...
    CDosEditListDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_Title = strTitle;
    dlg.m_Prompt = strPrompt;
    dlg.m_StrArray.Append(string_array);
...
int CDOSLibApp::ads_dos_popupmenu()
{
    ...
    POINT p2d;
    ::GetCursorPos(&p2d);
    CWnd* pWnd = CWnd::FromHandle(sds_getmainhwnd());
    if (pWnd)
...
int CDOSLibApp::ads_dos_msgboxex()
{
    ...
    CDosMessageDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
...
int CDOSLibApp::ads_dos_msgboxchk()
{
    ...
    CString str;
    str.Format(L"doslib_%ls", (const TCHAR*)strRegVal);
...
int CDOSLibApp::ads_dos_linetypebox()
{
    ...
    OdDbObjectId  old_linetypeId;
    if (!curDb())
        return RSERR;

    OdDbLinetypeTablePtr pTable = curDb()->getLinetypeTableId().openObject();
    if (pTable.isNull())
        return RSERR;

    for (auto iter = pTable->newIterator(); !iter->done(); iter->step())
    {
        auto record = iter->getRecord();
        if (record->getName().iCompare((const TCHAR*)strLinetype) == 0)
        {
            old_linetypeId = record->objectId();
        }
    }
    if (old_linetypeId.isNull())
    {
        old_linetypeId = curDb()->getCELTYPE();
    }
    OdDbObjectId new_linetypeId;
    if (drx_linetypeDialog(old_linetypeId, bInclude, new_linetypeId) && new_linetypeId)
    {
        OdDbLinetypeTableRecordPtr pSeleted = new_linetypeId.openObject();
        acedRetStr(pSeleted->getName());
    }
    else
    {
        acedRetNil();
...
int CDOSLibApp::ads_dos_orderlist()
{
    ...
    CDosOrderListDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_Title = strTitle;
    dlg.m_Prompt = strPrompt;
    dlg.m_StrArray.Append(strarray);
    if (dlg.DoModal() == IDOK)
...
int CDOSLibApp::ads_dos_duallist()
{
    ...
    CDosDualListDialog dlg(CWnd::FromHandle(sds_getmainhwnd()));
    dlg.m_title = strTitle;
    dlg.m_prompt = strPrompt;
