  ...
  //Get the working area for the primary monitor
    ...
    if (bNeedInitCode)
    {
        ...
        if (m_sTitle.GetLength())
        {
            ...
            m_rectTitle = CRect(std::max((long)5, m_rectIcon.right + 5), 5, m_rectClose.left - 5, 5 + sizeTitle.cy);
            nOffset = std::max(std::max(m_rectTitle.bottom + 5, m_rectClose.bottom + 5), m_rectIcon.bottom + 5);
